     ...
     internal class SerializedPropertyFilters
    {
     ...
     public class TreeViewUtility
     {
             ...
             TreeViewItem root)
         {
             ...
             for (int index = 0; index < visibleItems.Count; ++index)
             {
                 TreeViewUtility.SetChildParentReferences(index, visibleItems);
         ...
         {
             if (IsChildListForACollapsedParent((IList<TreeViewItem>) item.children) && newChildList == null)
                 return;
             item.children = newChildList;
         }
         //From LazyTreeViewDataSource
         public static bool IsChildListForACollapsedParent(IList<TreeViewItem> childList) => childList != null && childList.Count == 1 && childList[0] == null;
     }
}